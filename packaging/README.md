...
```powershell
.\scripts\build_release.ps1 -Version 0.68.0 -Clean -ForceRuntime
```
...
The runtime preparation step verifies developer/build dependencies with `pip check`, verifies the private runtime dependencies with `pip check`, prunes packaging-only `litellm` benchmark/static output files that can exceed classic Windows path limits, and then the release script checks that the installer file layout stays within the path budget before creating the ZIP or setup EXE.
